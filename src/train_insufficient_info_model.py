"""Fine-tune DistilBERT for insufficient-information classification."""
...
from config import DEFAULT_BASE_MODEL, DEFAULT_MAX_LENGTH, RANDOM_SEED, SPLIT_FILENAMES
from data_utils import load_labeled_jsonl, seed_everything, write_json
...
def make_dataset(records: list[dict]):
    ...
    return Dataset.from_list(
        [
            {
                "text": record["message"],
                "labels": int(record["insufficient_info"]),
            }
...
def compute_metrics(eval_pred) -> dict[str, float]:
    ...
    _, _, f1_by_class, _ = precision_recall_fscore_support(
        labels,
        predictions,
        labels=[0, 1],
        zero_division=0,
    ...
    return {
        ...
        "f1_sufficient": float(f1_by_class[0]),
        "f1_insufficient": float(f1_by_class[1]),
    }
...
def main() -> None:
    ...
    id2label = {0: "sufficient", 1: "insufficient"}
    ...
    write_json(
        {
            "task": "insufficient_information_binary_classification",
